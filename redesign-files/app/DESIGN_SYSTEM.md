...
The app has **two sidebar modes**: a main workspace rail and a settings sub-sidebar. The sidebar never nests more than two levels.

#### 3.10.1 Main rail (workspace pages)

Used on every workspace screen: Assistant, Conversations, Matters, Matter detail, Intakes, Invoices, Files, Clients, Calendar, Tasks, Trust, Reports (+ sub-report pages like Receivables, Time & Billing).

**Anatomy:**
- 240px wide, sticky, `100vh`
- 1px right border
- Brand mark at top, mono meta line ("Law Offices of Sarah Chen · solo")
- `.new-btn` (full-width primary — "＋ New conversation")
- Two `.rail-section` blocks:
  - **Workspace**: Assistant, Conversations, Matters, Intakes, Invoices, Files
  - **Practice**: Clients, Calendar, Tasks, Trust ledger, Reports
- Settings link pinned near bottom (hollow dot icon, `margin-top: auto`)
- User chip pinned to bottom
- Active item: `class="rail-item on"` → `--ink` background, accent text
- All items are `<a>` tags with `href` to their page

**This sidebar is identical on every workspace page.** Only the active item changes. Do not add page-specific items, conversation history, or contextual content to the main rail — that belongs in the page body or a right-side drawer.

#### 3.10.2 Settings sub-sidebar

Used on all `/settings/*` pages (17 pages across 4 sections).

**Anatomy:**
- 280px wide, sticky, `height: 100vh`, `overflow-y: auto`
- `← back to assistant` link at top (links to `Assistant.html`) — this is the mode-exit
- "Settings" heading with org name
- Four `.nav-section` blocks with mono labels:
  - **Practice**: Profile & areas, Team, Services & pricing
  - **Money**: Subscription, Stripe Connect, Trust accounting
  - **Intelligence**: AI behavior, Intake forms, Engagement templates, Apps & integrations, MCP & API
  - **Account**: Security, Sessions, Appearance, Notifications, Audit log, Export data
- Active item: `class="nav-item on"` → `--ink` background, accent text
- Collapses to `display: none` at `max-width: 980px`

**When to use which:**
- If the user is **doing work** (viewing matters, reading reports, managing files) → main rail
- If the user is **configuring the system** (changing AI behavior, managing integrations, account security) → settings sub-sidebar
- **No other pages** should swap the sidebar. If a page needs sub-navigation (e.g. report types), use in-page tabs instead.
