...
describe('worker route table', () => {
  ...
  it('does not register the worker-owned MCP routes (retired in favor of backend MCP)', () => {
    expectNoRoute('/api/mcp/internal/events');
    expectNoRoute('/api/mcp/ws');
    expectNoRoute('/api/mcp');
    expectNoRoute('/.well-known/oauth-protected-resource');
  });
