...
- The migration adds the column with `NULL` default; existing rows are unaffected; re-running the migration is a no-op (`CREATE … IF NOT EXISTS` semantics for the column-add path).
