...
Update bundler/tooling: add JSON locale alias in tsconfig.json:1, include .json glob in config/vitest/vitest.config.ts:1 resolve aliases, extend vite.config.ts:1 to pre-bundle i18next, mark locale chunk manual splitting, expose env var for available languages.
...
Email/PDF generators (worker/services/NotificationService.ts:1, worker/services/OneSignalService.ts:1, worker/services/PDFGenerationService.ts:1, worker/lib/pdf.ts:1) should load translated templates, with locale chosen via request metadata.
Docs, Build, Tests
