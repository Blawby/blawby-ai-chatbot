...
## 1) File Inventory (with line ranges)

### Infrastructure Components

#### `src/components/Modal.tsx` (167 lines)
- **Purpose**: Base modal component providing infrastructure (backdrop, animations, portal rendering, mobile drawer support)
- **Key exports**: `Modal` (default export)
- **Critical effects/state**:
  - Lines 35-50: `useEffect` for browser detection and mobile state
  - Lines 52-71: `useEffect` for keyboard (Escape) and body overflow handling when `isOpen` changes
- **Storage usage**: None
- **Modal-related imports**: None (base component)
- **Modal-related exports**: None (used by all other modals)

#### `src/components/onboarding/WelcomeModal.tsx` (128 lines)
- **Purpose**: Shows welcome tips after onboarding completion (NOT part of onboarding flow)
- **Key exports**: `WelcomeModal` (default export)
- **Critical effects/state**:
  - Lines 17-29: Component state (`isSubmitting`) and `handleComplete` handler
  - No internal effects - relies on parent for `isOpen` prop
- **Storage usage**: None (parent manages flags)
- **Modal-related imports**: `Modal` from `../Modal` (line 3)
- **Modal-related exports**: Used by `src/index.tsx:25`

### Informational Modals (to be moved)

#### `src/components/PricingModal.tsx` (366 lines)
- **Purpose**: Displays subscription pricing tiers, allows upgrades, manages billing portal
- **Key exports**: `PricingModal` (default export)
- **Critical effects/state**:
  - Lines 38-41: `useEffect` to load user's country preference
  - Lines 22-27: Props-based state (no internal modal trigger logic)
- **Storage usage**: None
- **Modal-related imports**: `Modal` from `./Modal` (line 5)
- **Modal-related exports**: Used by `src/index.tsx:24`

#### `src/components/ContactOptionsModal.tsx` (243 lines)
- **Purpose**: Shows contact options for lawyers (phone, email, website) with copy functionality
- **Key exports**: `ContactOptionsModal` (default export)
- **Critical effects/state**:
  - Lines 30: `copiedField` state for copy feedback
  - Lines 32-40: `copyToClipboard` function with 2s timeout
- **Storage usage**: None
- **Modal-related imports**: `Modal` from `./Modal` (line 4)
- **Modal-related exports**: Used by `src/components/LawyerSearchResults.tsx:4`

#### `src/components/CameraModal.tsx` (167 lines)
- **Purpose**: Camera capture interface for file uploads
- **Key exports**: `CameraIconModal` (default export, but component name is `CameraIconModal`)
- **Critical effects/state**:
  - Lines 72-79: `useEffect` to start/stop camera when `isOpen` changes
  - Lines 24-30: `stopCameraIcon` callback
  - Lines 32-69: `startCameraIcon` callback
- **Storage usage**: None
- **Modal-related imports**: `Modal` from `./Modal` (line 3)
- **Modal-related exports**: Used by `src/components/FileMenu.tsx:5`

### Onboarding Modals (stay in onboarding/)

#### `src/components/onboarding/BusinessOnboardingModal.tsx` (144 lines)
- **Purpose**: Multi-step business onboarding flow (welcome → firm basics → trust account → stripe → business details → services → review)
- **Key exports**: `BusinessOnboardingModal` (default export)
- **Critical effects/state**:
  - Lines 48-89: Loading, error, form state management
  - Lines 58-90: `handleStepContinue` with validation
  - Lines 92-100: `handleBack` navigation
- **Storage usage**: None
- **Modal-related imports**: `Modal` from `../Modal` (line 2)
- **Modal-related exports**: Used by `src/components/pages/BusinessOnboardingPage.tsx:3`, `src/components/onboarding/index.ts:2`

#### `src/components/onboarding/OnboardingModal.tsx` (194 lines)
- **Purpose**: Personal onboarding flow (personal info → use case)
- **Key exports**: `OnboardingModal` (default export)
- **Critical effects/state**:
  - Lines 40-67: `useEffect` to load existing onboarding data from session
  - Lines 101-153: `handleComplete` saves to server AND sets `localStorage.setItem('onboardingCompleted', 'true')` (line 119)
- **Storage usage**: 
  - **WRITES**: `localStorage.setItem('onboardingCompleted', 'true')` (line 119)
- **Modal-related imports**: `Modal` from `../Modal` (line 3)
- **Modal-related exports**: Used by `src/components/AuthPage.tsx:4`

### Main App Files

#### `src/index.tsx` (806 lines)
- **Purpose**: Main application entry point, manages routing, modals, session
- **Key exports**: `App`, `AppWithProviders`, `prerender`
- **Critical effects/state related to WelcomeModal**:
  - Line 58: `const [showWelcomeModal, setShowWelcomeModal] = useState(false);`
  - Lines 161-177: `useEffect` reads `localStorage.getItem('onboardingCompleted')` on mount and sets `showWelcomeModal(true)` if flag exists
  - Lines 211-278: `useEffect` syncs `onboardingCompleted` flag from session user object (lines 234-245) and sets it if missing (line 240)
  - Lines 435-448: `handleWelcomeComplete` clears flag and closes modal
