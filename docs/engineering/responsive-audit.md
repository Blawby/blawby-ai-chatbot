...
| -------------------------------------------------------------- | :----: | :----: | :-----: | ---------------------------------------------------------------------------------- |
| `src/app/MainApp.tsx`                                          |   —    |   —    |    ✅   | `layoutMode` enum: 'desktop' uses AppShell (audited here); 'mobile' (authenticated client on phone) renders a separate shell outside this audit's scope; 'widget' is covered in the Public widget shells section below |
| `src/shared/ui/layout/EmbedShell.tsx`                          |   ✅   |   ✅   |    ✅   | Transparent shell for `?v=widget` (embed iframe). Container width determined by iframe wrapper from `public/widget-loader.js` (`min(380px, calc(100vw - 40px))`); internal layout via `@container widget` on `widget-shell-gradient` |
| `src/shared/ui/layout/DirectShell.tsx`                         |   ✅   |   ✅   |    ✅   | Full-viewport shell for direct visits to `/public/:slug` and `/public/:slug/intake/:templateSlug`. No `overflow-hidden`, no `max-w-*`; widget fills the viewport. Mobile-first |
| `src/shared/ui/layout/MarketingShell.tsx`                      |   ✅   |   ✅   |    ✅   | Decorated dark-page + centered card for `/public/:slug/welcome`. `max-w-2xl` card on `bg-surface-app-frame`; overlays portal out via `WidgetOverlayRoot` so drawers paint across the full viewport |
