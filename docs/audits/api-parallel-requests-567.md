...
| 5  | `src/features/chat/pages/hooks/useWorkspaceSetup.ts` | `usePracticeManagement` + `usePracticeDetails` re-invoked inside the setup hook while `WorkspacePage`/`MainApp` already had them | Med | Accepted — `usePracticeManagement` uses a module-level `sharedPracticeSnapshot` and `usePracticeDetails` reads from the `practiceDetailsStore` nanostore. Both already share a single in-flight fetch + in-memory copy across all hook instances, so the apparent duplication is logical-only with no extra network traffic. Lifting props would not save any requests; verified via the `practiceDetailsStore` seeding path in `MainApp.tsx` (140–155) |
