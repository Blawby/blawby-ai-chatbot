...
- **Public-mode custom-template coverage is green, but currently bootstrap-stubbed**: `tests/e2e/intake-form-templates.spec.ts` runs in the public Playwright lane and verifies that `?template=<slug>` changes the resolved intake template, preserves structured custom field definitions, and embeds the selected template into conversation-create metadata.
- **Live template creation is currently blocked upstream**: even minimal docs-compliant calls to `POST /api/practice/{practice_id}/intake-templates` (for example `{ "name": "Probe Minimal", "slug": "probe-..." }`) return `500 INTERNAL_SERVER_ERROR` after validation on staging, so the public custom-template E2E cannot yet create/publish a real template end-to-end.
...
### 6. Convert stubbed public custom-template E2E to live builder flow once backend create works

**Current blocker:** The staging backend create endpoint for intake templates returns `500 INTERNAL_SERVER_ERROR` even for minimal documented payloads, so the browser test cannot yet create a real custom template before opening the public widget.

**When backend is fixed:**
- Replace the stubbed bootstrap payload in `tests/e2e/intake-form-templates.spec.ts` with real setup that creates and publishes a template through `POST /api/practice/{practiceId}/intake-templates`
- Assert the live public link generated by the builder (`/public/{slug}?template={templateSlug}`) resolves the exact saved template
- Add live assertions for structured field types (`select`, `multiselect`, `date`, `boolean`, `number`) so the widget flow verifies both routing and actual saved field definitions
