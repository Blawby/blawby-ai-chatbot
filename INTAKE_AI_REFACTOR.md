...
- **Backend PR #328 required**: the `?template_slug=` query param on `GET /{slug}/intake` is pending merge. Once merged, the E2E test should pass end-to-end.
...
**What's implemented:**
- Backend `GET /api/practice-client-intakes/{slug}/intake` now accepts optional `?template_slug=` query param — returns the matching published template for the practice, falls back to the practice default when omitted or not found (backend PR #328)
...
### 6. Convert stubbed public custom-template E2E to live builder flow ✅ done (pending backend PR #328)
...
- **Blocked on**: backend PR #328 merged and deployed to staging
