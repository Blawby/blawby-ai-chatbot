...
Backend API base: VITE_BACKEND_API_URL wins if set (used in prod or local providing). If VITE_BACKEND_API_URL is unset and VITE_ENABLE_MSW=true, the app uses Mock Service Worker (MSW) and will not call the staging URL (MSW intercepts/backfills API responses). If VITE_BACKEND_API_URL is unset and VITE_ENABLE_MSW is false (or unset), falls back to https://staging-api.blawby.com in dev. Source of truth for runtime host resolution is src/config/urls.ts.
