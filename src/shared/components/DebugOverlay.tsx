...
export const DebugOverlay: FunctionComponent<DebugOverlayProps> = ({ isVisible = false }) => {
 ...
 return (
  <div 
   className="fixed top-4 left-4 bg-surface-base bg-opacity-80 text-input-text p-4 rounded-lg text-xs font-mono max-w-sm z-50" 
  ...
  >
   <div className="mb-2 flex justify-between items-center">
    ...
    <button
     ...
     className="text-input-text hover:text-input-placeholder focus:outline-none focus:ring-2 focus:ring-white focus:ring-opacity-50 rounded px-1"
   ...
   {error && (
    <div className="mb-2 p-2 bg-[rgb(var(--error-foreground))] bg-opacity-50 rounded border border-[rgb(var(--error-foreground))]" role="alert" aria-live="assertive">
     <div className="text-[rgb(var(--error-foreground))] font-semibold">Error:</div>
     <div className="text-[rgb(var(--error-foreground))] text-xs">{error}</div>
    </div>
   )}
    ...
    {toolCalls.length === 0 ? (
     ...
     <div className="max-h-32 overflow-y-auto" role="list" aria-label="List of tool calls">
      {toolCalls.map((call, index) => (
       <div 
        key={index} 
        className="text-[rgb(var(--success-foreground))]" 
        role="listitem"
   ...
   {isExpanded && (
    <div className="mt-2 pt-2 border-t border-gray-600" role="region" aria-label="Additional debug information">
     <div className="text-xs text-input-placeholder">
      <div>Press <kbd className="bg-surface-base px-1 rounded">Esc</kbd> to collapse</div>
      <div>Press <kbd className="bg-surface-base px-1 rounded">Enter</kbd> or <kbd className="bg-surface-base px-1 rounded">Space</kbd> to toggle</div>
     </div>
    </div>
   )}
  </div>
