  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // ── 2. Persist anonymous identity hints ────────────────────────────
        // The authoritative widget session comes from the bootstrap payload and
        // is passed into WidgetApp directly. Keep only the storage hints used by
        // widget-token recovery; don't manually poke the Better Auth session.
