...
const CopyButton = ({ text }: { text: string }) => {
 ...
 return (
  <button
   type="button"
   onClick={handleCopy}
   className="absolute top-2 right-2 inline-flex items-center gap-1 rounded-md border border-white/10 bg-surface-base px-2 py-1 text-[11px] font-medium text-input-text opacity-0 transition focus-visible:opacity-100 group-hover:opacity-100"
...
export const markdownComponents: Components = {
 ...
 code({ className, children, ...props }) {
  ...
  if (isBlock) {
   return (
    <code className="block font-mono text-sm leading-relaxed text-input-placeholder" {...props}>
     {children}
    </code>
   );
  }
 ...
 pre({ children, ...props }) {
  ...
  return (
   <div className="group relative my-3 max-w-full min-w-0">
    ...
    <pre
     className="max-w-full min-w-0 overflow-x-auto rounded-lg p-4 bg-surface-base backdrop-blur-sm text-input-placeholder text-sm leading-relaxed"
