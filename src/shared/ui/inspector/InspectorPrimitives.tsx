        ...
        <div className="flex items-center justify-between px-5 pb-0.5 pt-2">
          ...
          {onToggle ? (
            <button
              ...
              className={`flex h-7 w-7 items-center justify-center rounded-md text-input-placeholder transition hover:bg-surface-utility/40 hover:text-input-text disabled:cursor-not-allowed disabled:opacity-50 ${isOpen ? 'bg-surface-utility/60 text-input-text' : ''}`}
            >
      ...
      <div className="flex items-start justify-between gap-1.5 group">
        ...
        {onToggle ? (
          <button
            ...
            disabled={disabled}
            aria-expanded={isOpen}
            className={`flex-shrink-0 ${label ? '-mt-1' : 'mt-0.5'} inline-flex h-7 w-7 items-center justify-center rounded-md text-input-placeholder transition hover:bg-surface-utility/40 hover:text-input-text disabled:cursor-not-allowed disabled:opacity-50 ${isOpen ? 'bg-surface-utility/60 text-input-text' : ''}`}
  ...
  return (
    <div className="flex flex-col items-center px-5 pb-4 pt-5">
      <Avatar 
        ...
        size="xl" 
        className="h-14 w-14"
        bgClassName="bg-surface-utility/40"
      />
...
export const InspectorHeaderHero = ({
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-[rgb(var(--accent-500))]/15 via-[rgb(var(--accent-500))]/5 to-transparent">
      <div className="absolute inset-0 bg-gradient-to-t from-surface-utility/40 via-transparent to-surface-workspace/15 dark:from-surface-utility/10 dark:via-transparent dark:to-surface-utility/5" />
      <div className="relative flex flex-col items-center px-5 pb-8 pt-10 text-center">
        ...
        <div className="relative">
          <Avatar 
            ...
            size="xl" 
            className="h-24 w-24"
            bgClassName="bg-surface-utility/40 shadow-2xl ring-1 ring-line-glass/10 ring-inset"
          />
          <div className="absolute inset-0 rounded-full border border-line-glass/10 pointer-events-none" />
        </div>

        {/* Name & Description */}
        <div className="mt-6 min-w-0 max-w-full">
          <h3 className="text-2xl font-bold tracking-tight text-input-text">{name}</h3>
          {subtitle ? (
            <p className="mt-2 text-[13px] leading-relaxed text-input-placeholder line-clamp-3 px-2">{subtitle}</p>
          ) : null}
        </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          {phone && (
            <button
               ...
               className="flex h-10 w-10 items-center justify-center rounded-full bg-accent-500/10 text-accent-600 transition hover:bg-accent-500/20 hover:text-accent-700 dark:text-accent-400"
            >
  ...
  return (
    <button
      ...
      className={`w-full py-2.5 text-center text-[13px] font-medium transition-opacity hover:opacity-70 active:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ${
        destructive
          ? 'text-[rgb(var(--error-foreground))] focus-visible:ring-[rgb(var(--error-foreground))]/50'
