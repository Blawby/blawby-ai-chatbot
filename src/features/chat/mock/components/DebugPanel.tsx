    ...
    <div className="border-t border-gray-200 dark:border-dark-border bg-white dark:bg-dark-bg h-72 flex flex-col">
      ...
      >
        ...
        <span className="flex items-center gap-2">
          <span className="text-xs rounded-full bg-gray-100 dark:bg-dark-card-bg px-2 py-1 text-gray-600 dark:text-gray-300">{events.length}</span>
      ...
      {isOpen && (
        <>
          ...
          <div className="flex-1 overflow-y-auto px-4 pb-4 space-y-3 text-xs font-mono">
            {events.map((event) => (
              <div key={event.id} className="border border-gray-200 dark:border-dark-border rounded p-3 bg-gray-50 dark:bg-dark-card-bg">
