      ...
      <div className="glass-panel p-4 rounded-lg">
       ...
       <div className="w-48 h-48 bg-surface-base rounded flex items-center justify-center">
        <div className="text-center">
         <div className="w-32 h-32 bg-surface-base dark:bg-white rounded grid grid-cols-8 gap-1 p-2">
          {/* Mock QR pattern - stable pattern that doesn't flicker */}
          {qrPattern.map((isWhite, i) => (
           <div
            key={i}
            className={`w-full h-full rounded-sm ${
             isWhite ? 'bg-white dark:bg-surface-base' : 'bg-surface-base dark:bg-white'
            }`}
           />
          ))}
         </div>
