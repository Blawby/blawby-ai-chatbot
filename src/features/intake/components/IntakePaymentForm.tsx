...
export const IntakePaymentForm: FunctionComponent<IntakePaymentFormProps> = ({
 ...
 return (
  <form onSubmit={handleSubmit} className="space-y-5">
   ...
   {errorMessage && (
    <div className="rounded-xl border border-[rgb(var(--error-foreground))]/30 bg-[rgb(var(--error-foreground))]/10 px-4 py-3 text-sm text-[rgb(var(--error-foreground))] backdrop-blur-xl dark:text-[rgb(var(--error-foreground))]">
     {errorMessage}
    </div>
   )}

   {callbackWarning && (
    <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-[rgb(var(--warning-foreground))] backdrop-blur-xl ">
     {callbackWarning}
    </div>
   )}
