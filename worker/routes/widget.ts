...
const isValidCondition = (
  value: unknown,
): value is NonNullable<IntakeTemplate["fields"][number]["condition"]> => {
...
const normalizeCrossSiteWidgetCookie = (
  cookie: string,
  request: Request,
): string => {
...
export async function handleWidgetBootstrap(
  request: Request,
  env: Env,
): Promise<Response> {
  ...
  const getPracticeDetails = (async () => {
    const res = await RemoteApiService.getPublicPracticeDetails(
      env,
      slug,
      request,
    );
    if (!res.ok) {
      ...
      throw new Error(
        `[Bootstrap] Error fetching practice details: ${res.status} - ${text}`,
      );
    }
    try {
      ...
      throw new Error(
        `[Bootstrap] Failed to parse JSON from upstream practice details: ${parseErr instanceof Error ? parseErr.message : String(parseErr)}`,
      );
    }
  })();
  const requestedTemplateSlug = asNonEmptyString(
    url.searchParams.get("template"),
  );
  const getIntakeSettings = (async () => {
    const res = await RemoteApiService.getPublicPracticeIntakeSettings(
      env,
      slug,
      request,
    ...
    if (!res.ok) {
      ...
      const text = await res.text().catch(() => "No body");
      throw new Error(
        `[Bootstrap] Error fetching intake settings: ${res.status} - ${text}`,
      );
    }
    try {
      return await res.json();
      ...
      throw new Error(
        `[Bootstrap] Failed to parse JSON from upstream intake settings: ${parseErr instanceof Error ? parseErr.message : String(parseErr)}`,
      );
    }
  })();
  ...
  let cookieSessionData: {
    user?: { id?: string; is_anonymous?: boolean };
    session?: { id?: string };
  } | null = null;
  let tokenSessionData: {
    user?: { id?: string; is_anonymous?: boolean };
    session?: { id?: string };
  } | null = null;
  ...
  try {
    ...
    try {
      const sessionRes = await fetch(
        `${env.BACKEND_API_URL}/api/auth/get-session`,
        {
    ...
    Logger.warn("[Bootstrap] Session check error ignored", {
  ...
  if (requestedWidgetToken) {
    try {
      const validatedToken = await validateWidgetAuthToken(
        requestedWidgetToken.token,
        env,
      );
      ...
      if (
        cookieSessionData?.user?.id &&
        tokenSessionData?.user?.id &&
        cookieSessionData.user.id !== tokenSessionData.user.id
      ) {
        Logger.info(
          "[Bootstrap] Identity mismatch detected; discarding stale widget token",
          {
    ...
    try {
      ...
      try {
        const anonRes = await fetch(
          `${env.BACKEND_API_URL}/api/auth/sign-in/anonymous`,
          {
            method: "POST",
            headers: anonHeaders,
            body: "{}",
        ...
        if (!anonRes.headers.getSetCookie) {
          throw new Error(
            "[Bootstrap] Environment does not support getSetCookie(); cannot reliably extract Set-Cookie headers.",
          );
        }
  ...
  const intakeSettingsPayload = (await getIntakeSettings) as Record<
    string,
    unknown
  >;
  const pd = (
  ...
  if (!practiceId) {
    throw HttpErrors.badGateway(
      "Unable to resolve practice id from practice details",
    );
  }
  ...
  const sessionId =
    typeof typedSessionDataResolved?.session?.id === "string" &&
    typedSessionDataResolved.session.id.trim().length > 0
    ...
    isAnonymous && sessionUserId && sessionId
      ? await issueWidgetAuthToken(
          ...
          validatedWidgetTokenSource
            ? {
                ttlSeconds: getWidgetTokenTtlForSource(
                  validatedWidgetTokenSource,
                ),
              }
            : undefined,
  ...
  if (practiceId && sessionUserId) {
    try {
      ...
      console.error("[Bootstrap] Failed to get conversations", {
        sessionUserId,
        practiceId,
        error: err,
      });
  ...
  // ------------------------------------------------------------------
  const rawIntakeTemplate = intakeData.intake_template as
    | BackendIntakeTemplatePublic
    | null
    | undefined;
  if (!rawIntakeTemplate) {
    throw HttpErrors.badGateway(
      `[Bootstrap] No published intake template found for practice '${slug}'. Ensure the practice has a published default template.`,
    );
  }
  ...
  const preSelectedServiceUuid = asNonEmptyString(
    ...
    fields: (rawIntakeTemplate.fields ?? [])
      ...
      .map((f) => {
        const rules =
          ...
          !Array.isArray(f.validation_rules)
            ? (f.validation_rules as Record<string, unknown>)
            : {};
        return {
          key: f.key,
          label: f.label,
          type: (f.field_type === "textarea" ||
          f.field_type === "email" ||
          f.field_type === "phone"
            ? "text"
            : f.field_type === "multiselect"
              ? "select"
              : f.field_type) as IntakeTemplate["fields"][number]["type"],
          required: f.required,
          phase: f.phase,
          ...
          condition: isValidCondition(rules.condition)
            ? rules.condition
            : undefined,
          completenessWeight:
            typeof rules.completeness_weight === "number"
              ? rules.completeness_weight
              : undefined,
        };
      }),
  };
  ...
  const missingLocked = STANDARD_FIELD_DEFINITIONS.filter(
    (f) => LOCKED_KEYS.has(f.key) && !presentKeys.has(f.key),
  );
  ...
  for (const cookie of responseCookies) {
    responseHeaders.append(
      "Set-Cookie",
      normalizeCrossSiteWidgetCookie(cookie, request),
    );
  }
...
export async function handlePublicPracticeIntakeSettings(
  request: Request,
  env: Env,
): Promise<Response> {
  if (request.method !== "GET") {
  ...
  const match = url.pathname.match(
    /^\/api\/practice-client-intakes\/([^/]+)\/intake$/,
  );
  ...
  if (!res.ok) {
    ...
    const text = await res.text().catch(() => "No body");
    throw HttpErrors.badGateway(
      `[Public intake settings] Error fetching intake settings: ${res.status} - ${text}`,
    );
  }

  const intakeSettingsPayload = (await res.json().catch(() => null)) as Record<
    string,
    unknown
  > | null;
  ...
  const settingsRecord =
    dataRecord?.settings && typeof dataRecord.settings === "object"
      ? (dataRecord.settings as Record<string, unknown>)
      : intakeSettingsPayload.settings &&
          typeof intakeSettingsPayload.settings === "object"
        ? (intakeSettingsPayload.settings as Record<string, unknown>)
        : {};
  const organizationRecord =
    dataRecord?.organization && typeof dataRecord.organization === "object"
      ? (dataRecord.organization as Record<string, unknown>)
      : intakeSettingsPayload.organization &&
          typeof intakeSettingsPayload.organization === "object"
        ? (intakeSettingsPayload.organization as Record<string, unknown>)
        : {};
  const intakeTemplate =
    (dataRecord?.intake_template as unknown) ??
    intakeSettingsPayload.intake_template ??
    null;
  ...
  const organization = {
    ...
    name:
      typeof organizationRecord.name === "string"
        ...
        : undefined,
    logo:
      typeof organizationRecord.logo === "string"
