...
export async function handleAuthProxy(request: Request, env: Env): Promise<Response> {
  ...
  // DEV: add non-sensitive debug headers when DEBUG is enabled.
  // These headers surface cookie *names* and attribute metadata only (no cookie values).
  try {
    const enabled = String(env.DEBUG).toLowerCase() === 'true' || String(env.DEBUG) === '1' || String(env.ALLOW_DEBUG) === 'true';
    if (enabled) {
      // Parse Set-Cookie headers from the original response (if any)
      const rawSetCookie = (response.headers as Headers & { getSetCookie?: () => string[] }).getSetCookie?.() ?? [];
      // Fallback to single header string when getSetCookie() is not available
      if (rawSetCookie.length === 0) {
        const sc = response.headers.get('Set-Cookie');
        if (sc) {
          // Attempt naive split; may join multiple cookies in some runtimes.
          rawSetCookie.push(...sc.split(/,(?=\s*[^=]+=)/));
        }
      }
      ...
      // Also report which cookie names were sent by the client in the incoming request
    ...
    // Do not fail the request on debug header assembly errors
    console.warn('[authProxy] failed to add debug headers', err);
  }
...
export async function handleBackendProxy(request: Request, env: Env): Promise<Response> {
  ...
  // DEV: add non-sensitive debug headers when DEBUG is enabled (backend proxy branch)
  try {
    const enabled = String(env.DEBUG).toLowerCase() === 'true' || String(env.DEBUG) === '1' || String(env.ALLOW_DEBUG) === 'true';
    if (enabled) {
      const rawSetCookie = (response.headers as Headers & { getSetCookie?: () => string[] }).getSetCookie?.() ?? [];
    ...
    console.warn('[backendProxy] failed to add debug headers', err);
  }
